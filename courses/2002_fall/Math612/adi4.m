%ADI - 2D - Dirichlet BCs on two sides (homogeneous and nonhomogenous)
% and Neumann on two sides.

% The nonhomogenous BC is u(x(end),y) = 0.25.

function u = adi4(x,y,T,kappa,u0)
...
%Impose the nonhomogeneous BC.

f = [zeros(1,m-1) mux*0.25]';
F = kron(ones(n,1),f);
...
u(:,:,1) = reshape(v1,m,n)';
for t=T(2:end)
    v2 = (speye(m*n)-Dxx)\((speye(m*n)+Dyy)*v1+F);
    v1 = (speye(m*n)-Dyy)\((speye(m*n)+Dxx)*v2+F);
